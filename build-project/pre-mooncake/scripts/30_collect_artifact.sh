...
#   2. 缺失检查升级：缺失时输出每条 missing 的具体行（满足 [3] 末尾的 TODO）；
...
ldd "$BINARY" \
    ...
    | sort -u \
    | xargs -I{} cp -L {} "$LIB_DIR/"
log_info "已收集 ldd 可见的运行时 .so"

# ---- 3) 强制覆盖 GCC12 的 libstdc++ / libgcc_s ----------------------------
# 即便 ldd 已带过来，也要"覆盖"为 GCC12 路径下的版本（避免 ldd 解析到 OS 老版）。
cp -Lv "${GCC_TOOLCHAIN_PREFIX}/lib64/libstdc++.so.6" "$LIB_DIR/"
cp -Lv "${GCC_TOOLCHAIN_PREFIX}/lib64/libgcc_s.so.1"  "$LIB_DIR/"

# ---- 4) 单独补 dlopen 加载的 libetcd_wrapper.so ---------------------------
ETCD_WRAPPER=/usr/local/lib/libetcd_wrapper.so
[[ -f "$ETCD_WRAPPER" ]] || die "$ETCD_WRAPPER 不存在"
cp -L "$ETCD_WRAPPER" "$LIB_DIR/"

# ---- 5) 缺失检查（含明细） ------------------------------------------------
log_info "=== 收集清单 ==="
ls -lh "$LIB_DIR/"

log_info "=== 缺失检查（模拟 Runtime: LD_LIBRARY_PATH=lib/）==="
missing="$(LD_LIBRARY_PATH="$LIB_DIR" ldd "$BIN_DIR/mooncake_master" | grep "not found" || true)"
if [[ -n "$missing" ]]; then
    log_error "缺失以下动态库："
    printf '%s\n' "$missing" | sed 's/^/    /' >&2
    die "运行时依赖不完整"
...
gcc_toolchain  : $(${GCC_TOOLCHAIN_PREFIX}/bin/gcc --version | head -n1)
